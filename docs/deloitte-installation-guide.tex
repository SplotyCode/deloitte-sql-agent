...
\begin{quote}
\texttt{https://deloitteemea.service-now.com/mysupport?id=sc_cat_item&sys_id=4e02d262c3a7c2145608282ce00131bf&table=sc_cat_item&searchTerm=VMware%20Workstation%20Player}
\end{quote}

\section{Step 1: Install uv}

uv is the Python package manager used by this project. Install it from the official Astral installer.\footnote{\url{https://docs.astral.sh/uv/getting-started/installation/}}
...
If the installer script is blocked in your environment, use the official WinGet package instead:

\begin{lstlisting}
winget install --id=astral-sh.uv -e
\end{lstlisting}

\screenshotplaceholder{Add a screenshot of the uv installation command and successful \texttt{uv --version} output.}
...
\screenshotplaceholder{Add a screenshot of the Python installation step.}
...
\screenshotplaceholder{Add a screenshot of the \texttt{uv --native-tls sync} command running successfully.}
...
\subsection*{5.2 Open the keys page}
...
\subsection*{5.3 Create and copy the token}

\begin{enumerate}[leftmargin=1.5em]
    \item Click the button to create a new API key.
    \item Give the key a clear name, for example \texttt{Deloitte SQL Agent}.
    \item If your OpenRouter workspace requires budget or billing configuration, complete that step.
...
\screenshotplaceholder{Add a screenshot of the OpenRouter sign-in page.}
\screenshotplaceholder{Add a screenshot of the OpenRouter API keys page and the create-key dialog.}
...
\screenshotplaceholder{Add a screenshot of a successful first run of the SQL Agent.}
