...
\begin{enumerate}[leftmargin=1.5em]
    \item Open SharePoint.
    \item Navigate to \textbf{Qualit\"atsmanagement} $\rightarrow$ \textbf{Testdatenmanagement} $\rightarrow$ \textbf{dump subset}.
    \item Download the ZIP file \texttt{agent.zip}.
...
\screenshot{download.png}{SharePoint download page for the Deloitte SQL Agent ZIP package.}
...
\section{Optional Step: Create a Sample SQLite Database for a Test Run}

If you want to validate the installation before using a real source database, you can generate a local SQLite sample database inside the project folder.

Run:

\begin{lstlisting}
uv --native-tls run python generate_sample_dump.py `
  --db-filename sample_dump.db `
  --scenario support `
  --scale tiny `
  --hidden-reference-mode light `
  --skip-sql-dump
\end{lstlisting}

This creates a file named \texttt{sample\_dump.db} in the current directory. The \texttt{tiny} scale is intended for a quick setup check and keeps the test run fast.

For additional fixture-generator options and larger scenarios, see \texttt{docs/usage.md} in the project ZIP.

\section{Step 6: Run the SQL Agent}

Run the agent from the project folder. The example below uses the sample SQLite database created in the optional step above. If you already have a real database, replace the value of \texttt{--db-url}.

\begin{lstlisting}
uv --native-tls run python main.py `
  --db-url sqlite:///C:/Users/<your-user>/Projects/deloitte-sql-agent/sample_dump.db `
  --target-rows 1000 `
  --out subset.sql `
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item For the full planner command reference and additional arguments, see \texttt{docs/usage.md} in the project ZIP.
\end{itemize}
